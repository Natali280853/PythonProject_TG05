...
@dp.message()  # здесь ловим все сообщения
async def send_cat_info(message: Message):
    breed_name = message.text  # функция будет брать информацию об указанной породе
    breed_info = get_breed_info(breed_name)  # переменная сохраняет полученную информацию
    if breed_info:
        cat_image_url = get_cat_image_by_breed(breed_info['id'])  # id породы уже есть в breed_info
        # Формируем сообщение с информацией, которую хотим отправить
        info = (
        ...
        # Настраиваем отправку информации:
